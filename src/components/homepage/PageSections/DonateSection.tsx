...
function DonationForm() {
  ...
  return (
    <form
      ...
      className="flex w-full flex-col gap-y-6 rounded-3xl bg-teal-100 px-8  py-6 shadow-md shadow-stone-400 lg:max-w-[600px]"
    >
      <p className="font-handwriting text-3xl font-light text-teal-950 lg:my-2 lg:w-full lg:text-center lg:text-5xl ">
        Begin Contributing
      </p>
      <div className=" flex w-full items-center justify-center gap-x-2">
        {possibleDurations.map((duration) => (
          <button
            type="button"
            className={`w-full  rounded-md px-10 py-1 text-teal-900 shadow-md  shadow-stone-400 hover:text-black ${chosenDuration === duration ? `bg-cyan-200` : `bg-white transition-all duration-300 `} `}
            key={duration}
      ...
      <div className="flex w-full justify-center gap-1">
        {possibleAmounts.map((amount) => (
          <button
            type="button"
            className={`w-full rounded-md px-6 py-1 text-sm text-teal-900 shadow-md  shadow-stone-400 hover:text-black ${chosenAmount === amount ? `bg-cyan-200` : `bg-white transition-all duration-300 `} `}
            key={amount}
            onClick={() => {
      ...
      className={`min-h-[60px] w-full`}>
        {chosenAmount === 'custom' && (
          <div className="relative mt-4 w-full ">
            <div className="flex h-8 max-w-[200px] rounded-md shadow-md shadow-stone-400 ">
              <select
                {...register('currency')}
                className="h-full w-12 rounded-l-md bg-cyan-100 text-xs text-cyan-500 "
              >
              ...
              <input
                ...
                className="h-full w-full  rounded-r-md pl-2"
              />
            </div>
            <p className="absolute right-0 top-0 text-right text-xs text-cyan-600  ">
      ...
      <p className="w-full text-cyan-800">
        {`
      ...
      <button
        type="submit"
        className="mx-auto mt-4 rounded-3xl bg-cyan-500 px-12 py-2 font-handwriting text-2xl text-cyan-900 transition-all duration-300 hover:bg-cyan-400"
      >
        Donate
      </button>
    </form>
